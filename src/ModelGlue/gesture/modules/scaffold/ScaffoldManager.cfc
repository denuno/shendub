<cfcomponent output="false">
	...
	<cffunction name="generate" output="false" access="public" returntype="void" hint="I generate the scaffolds and load them into the model glue memory space">
		...
		<cfloop from="1" to="#arrayLen( arguments.scaffolds )#" index="i">
			<cfset arrayAppend( inflatedScaffoldArray, newScaffold( arguments.scaffolds[i].type, _ormAdapter.getObjectMetadata( arguments.scaffolds[i].object ), arguments.scaffolds[i].propertylist, arguments.scaffolds[i].eventType )) />
		</cfloop>
...
') />
	...
	<cffunction name="newBean" output="false" access="private" returntype="any" hint="I make a new instance of a scaffoldbean">
	...
	<input type="hidden" id="#pk#" name="#pk#" value="###arguments._alias#Record.get#pk#()##">', " ") />
		</cfloop>
